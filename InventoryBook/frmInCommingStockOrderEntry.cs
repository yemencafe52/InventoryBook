...
{
    public partial class frmInCommingStockOrderEntry : Form
    {
        public frmInCommingStockOrderEntry()
        {
            InitializeComponent();
            Preparing();
        }
        ...
        private void Save()
        {
            ...
            for(int i=0;i<listView1.Items.Count;i++)
            {
                ...
                items.Add(new ItemMovement(0, item.Number, (uint)numericUpDown1.Value ,byte.Parse(listView1.Items[i].SubItems[3].Text), 0, byte.Parse(listView1.Items[i].SubItems[4].Text), 0));
            }

            StockDocument sd = new StockDocument((uint)numericUpDown1.Value, dateTimePicker1.Value, (byte)numericUpDown2.Value, textBox1.Text, items, DocumentType.IncommingStockOrder);
        ...
        private void New()
        {
            ...
            numericUpDown1.Value = new StockDocumentController(new dbmgrlib.AccessDB(Constatns.ConnectionString)).GenerateNewDocumentNumber(DocumentType.IncommingStockOrder);
        ...
        private void Display(StockDocument document)
        {
            ...
            for(int i=0;i<document.Items.Count;i++)
            {
                ...
                lvi.SubItems.Add(document.Items[i].ItemMovmentIN.ToString());
                lvi.SubItems.Add(document.Items[i].ItemMovementBitIN.ToString());
        ...
        private void numericUpDown1_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (new StockDocumentController(new dbmgrlib.AccessDB(Constatns.ConnectionString)).Query((uint)numericUpDown1.Value, ref sd, DocumentType.IncommingStockOrder))
                {
                    Display(sd);
                }
            }
